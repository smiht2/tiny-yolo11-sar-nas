...
def score_s3(rec, mb, Lb, Lstar, Sstar, lam_m=1.0, lam_L=1.0, lam_S=1.0, rho_aug=0.2, hard_caps=True):
    ...
    if hard_caps and (L > Lstar or S > Sstar): return -1e9
    mt = max(1e-9, m/mb); Lt = max(1e-9, Lb/L); St = max(1e-9, Sstar/S)
    gaps = [lam_m*abs(1.0-mt), lam_L*abs(1.0-Lt), lam_S*abs(1.0-St)]
    worst = max(gaps)
    linear = lam_m*(1.0-mt) + lam_L*(1.0-Lt) + lam_S*(1.0-St)
    return -worst - rho_aug*linear

def main():
    p = build_parser("runs/nas_scalar_s3")
    p.add_argument("--lam_m", type=float, default=1.0)
    p.add_argument("--lam_L", type=float, default=1.0)
    p.add_argument("--lam_S", type=float, default=1.0)
    p.add_argument("--rho_aug", type=float, default=0.2)
    p.add_argument("--hard_caps", action="store_true")
    p.add_argument("--mutation_prob", type=float, default=0.2)
    args = p.parse_args()

    def score_fn(rec, mb, Lb, Lstar, Sstar):
        return score_s3(rec, mb, Lb, Lstar, Sstar, lam_m=args.lam_m, lam_L=args.lam_L, lam_S=args.lam_S, rho_aug=args.rho_aug, hard_caps=args.hard_caps)

    run_search(args, score_fn, "s3")
