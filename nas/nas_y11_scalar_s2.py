...
def score_s2(rec, mb, Lb, Lstar, Sstar, eta=-0.5, w_m=0.5, w_L=0.3, w_S=0.2, kappa=10.0, mu=10.0):
    ...
    if abs(eta) < 1e-9:
        base = (mt**w_m) * (Lt**w_L) * (St**w_S)
    else:
        base = (w_m*(mt**eta) + w_L*(Lt**eta) + w_S*(St**eta))**(1.0/eta)

    barrier = math.exp(-kappa*_softplus(L/Lstar - 1.0)) * math.exp(-mu*_softplus(S/Sstar - 1.0))
    return base * barrier

def main():
    p = build_parser("runs/nas_scalar_s2")
    p.add_argument("--eta", type=float, default=-0.5)
    p.add_argument("--w_m", type=float, default=0.5)
    p.add_argument("--w_L", type=float, default=0.3)
    p.add_argument("--w_S", type=float, default=0.2)
    ...
    def score_fn(rec, mb, Lb, Lstar, Sstar):
        return score_s2(rec, mb, Lb, Lstar, Sstar, eta=args.eta, w_m=args.w_m, w_L=args.w_L, w_S=args.w_S, kappa=args.kappa, mu=args.mu)

    run_search(args, score_fn, "s2")
